...
CREATE TABLE [KhoaMonHoc] (
    [MaKhoa] int NOT NULL,
    [MaMonHoc] int NOT NULL,
    CONSTRAINT [PK_KhoaMonHoc] PRIMARY KEY ([MaKhoa], [MaMonHoc]),
    CONSTRAINT [FK_KhoaMonHoc_Khoa_MaKhoa] FOREIGN KEY ([MaKhoa]) REFERENCES [Khoa] ([MaKhoa]) ON DELETE CASCADE,
    CONSTRAINT [FK_KhoaMonHoc_MonHoc_MaMonHoc] FOREIGN KEY ([MaMonHoc]) REFERENCES [MonHoc] ([MaMonHoc]) ON DELETE CASCADE
);
GO
...
CREATE INDEX [IX_KhoaMonHoc_MaMonHoc] ON [KhoaMonHoc] ([MaMonHoc]);
GO
...
CREATE INDEX [IX_LopQuanLi_MaGiangVien] ON [LopQuanLi] ([MaGiangVien]);
GO
...
VALUES (N'20230802030328_tao_bang', N'7.0.9');
GO

COMMIT;
GO;

-- ALTER TABLE [MonHoc] ALTER COLUMN [MaMonTienQuyet] int;
-- GO